...
static void epoll_handle_main_sock(struct worker_state *self)
{
	...
	while (rep--) {
		...
		if (n < 0) {
			if (errno == EAGAIN || errno == EWOULDBLOCK) {
				...
				epoll_send_arm(self, conn, events);
				return;
			}
			warn("Send failed");
			worker_kill_conn(self, conn);
		...
		if (!conn->to_send && !conn->to_send_comp) {
			epoll_send_disarm(self, conn, events);
...
epoll_handle_recv(struct worker_state *self, struct worker_connection *conn)
{
	...
	while (rep--) {
		...
		if (self->rx_mode == KPM_RX_MODE_DEVMEM)
			...
			n = epoll_handle_regular_recv(self, conn, chunk, rep,
						       self->validate);
		if (n == 0) {
			warnx("zero recv");
			worker_kill_conn(self, conn);
...
epoll_handle_conn(struct worker_state *self, int fd, unsigned int events)
{
	...
	if (events & EPOLLOUT) {
		if (conn->to_send)
			epoll_handle_send(self, conn, events);
		else if (!conn->to_send_comp)
			epoll_send_disarm(self, conn, events);
	}
	if (events & EPOLLIN) {
		if (conn->to_recv) {
			epoll_handle_recv(self, conn);
	...
	if (events & EPOLLERR)
		epoll_handle_completions(self, conn, events);

	if (!(events & (EPOLLOUT | EPOLLIN | EPOLLERR)))
		warnx("Connection has nothing to do %x", events);
}

static void epoll_prep(struct worker_state *self)
{
...
static void epoll_wait_(struct worker_state *self, int msec)
{
	...
	for (i = 0; i < nfds; i++) {
		...
		if (e->data.fd == self->main_sock)
			epoll_handle_main_sock(self);
		else
			epoll_handle_conn(self, e->data.fd,
						e->events);
	}
}
